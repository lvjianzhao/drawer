    ...
    items: [
      {
        ...
        itemRender: {
          name: "$input",
          props: {placeholder: "输入客户名称", clearable: true}
        }
      },
      {
        field: "serviceStatus",
      ...
      query: ({page, form}: VxeGridPropTypes.ProxyAjaxQueryParams) => {
        ...
        return new Promise<any>((resolve: Function) => {
          ...
          const params = {
            ...
            /** 默认serviceStatus为2,表示包括所有状态(因为后端无法区分空值和0的区别)*/
            serviceStatus: form.serviceStatus || "2",
...
const crudStore = reactive({
  ...
  onShowModal: (row?: RowMeta) => {
    if (row) {
      ...
      xFormOpt.data.businessName = row.businessName
      ...
      if (row.maintenanceType != 0) {
        xFormOpt.data.maintenanceType = row.maintenanceType
      } else {
        xFormOpt.data.maintenanceType = ""
      }
    } else {
      xFormOpt.data.id = 0
  ...
  onSubmitForm: () => {
    ...
    xFormDom.value?.validate(async (errMap?: VxeFormDefines.ValidateErrorMapParams) => {
      ...
      try {
        ...
        const mt = showMaintenanceType.value === true ? xFormOpt.data.maintenanceType : 0

        const res = await apiFunction({
          id: xFormOpt.data.id,
          name: xFormOpt.data.name,
          type: xFormOpt.data.type,
          businessName: xFormOpt.data.businessName,
