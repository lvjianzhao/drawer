          ...
          <el-table-column prop="active" label="状态" align="center">
            <template #default="{ row }">
              <el-switch
                ...
                active-text="启用"
                inactive-text="禁用"
                @change="switchAction(row.ID, row.active)"
                :disabled="row.username === 'admin' && row.role === 'admin'"
              />
            </template>
          </el-table-column>
          <el-table-column fixed="right" label="操作" align="center" min-width="200px">
            <template #default="{ row }">
              <el-button type="primary" text icon="Edit" size="small" @click="editDialog(row)">编辑</el-button>
              <el-button type="primary" text icon="Key" size="small" @click="modifyDialog(row)">修改密码 </el-button>
              <el-button
                type="danger"
                text
                icon="Delete"
...
const editDialog = (row: UsersResponse) => {
  formData.isdisabled = row.username === "admin" && row.role === "admin" ? true : false
  activeRow = row
